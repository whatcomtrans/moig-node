        ...
        return _this._executeEvent(params, callback, "advGetEvent", `{
            ...
            "eventData": {
                ...
                "callEvent": {
                    ...
                    "objectId": "{{callEvent.attributes.objectId}}"{{#type}},
                    "type": "{{type}}" {{/type}}{{#cause}}, 
                    "cause": "{{cause}}" {{/cause}}
                }{{/callEvent}}
            }{{/eventType}}
        }`) // TODO: this template needs testing and further development
    ...
    _executeEvent(params, callback, methodName, returnTemplate) {
        ...
        .then(function(ret) {
            ...
            if (err.message == "Timeout no event message available.") {
                return Promise.resolve({"success": false, "error": err, "result": {}, "methodName": methodName, "timeout": true, "eventType": "timeout"})
            } else {
                console.log(err)
                return err
            }
        ...
        .then(function(ret) {
            // emit
            if (ret.eventType != "timeout") {
                _this.emit(ret.methodName, ret)
            }
            _this.emit(ret.eventType, ret)
            // callback
            callback(null, ret)
            // Return result to promise
            return ret
        ...
        return _this._soap_client[methodName + "Async"](soapParams).then(function(result){
            ...
            if (result.return.attributes.result === "true") {
                ...
                if (resultTemplate != null) {
                    ...
                    // TODO: debug only
                    console.log("------------" + methodName + " resultObj---------------------")
